...
func (rf *Raft) electionStart() (ret bool) {
	rf.rw.Lock()
	//把自己变为候选人
	rf.state = Candidate
	//自身任期加加
	...
	for i := 0; i < len(rf.peers); i++ {
		...
		//不要再协程之前重置心跳会变得不幸
		go func(server int, args *RequestVoteArgs, reply *RequestVoteReply, VoteN *int) {
			rf.sendRequestVote(server, args, reply, VoteN)
		}(i, &args, &reply, &VoteN)
	}
	//等待投票结果,世界可能变天
	time.Sleep(100 * time.Millisecond)
	rf.rw.Lock()
	if rf.currentTerm != currentTerm || rf.killed() {
		rf.rw.Unlock()
		return false
	}
	rf.rw.Unlock()
	if rf.getState() == Follower {
		PrettyDebug(dTimer, "[	    func-electionStart-rf(%+v)		] : 我变成打工人了,选举任期rf.Term: %v,得到票数:%v,为谁投票了%v\n",
			rf.me, rf.currentTerm, VoteN, rf.votedFor)
		return false
	}
	go rf.refreshElected()
	if VoteN > len(rf.peers)/2 {
		PrettyDebug(dTimer, "[	    func-electionEvent-rf(%+v)		] : 我选举成功了,选举任期rf.Term: %v,得到票数:%v,为谁投票了%v\n",
			rf.me, rf.currentTerm, VoteN, rf.votedFor)
		return true
	}
...
func (rf *Raft) sendRequestVote(server int, args *RequestVoteArgs, reply *RequestVoteReply, VoteN *int) {
	if rf.killed() {
		PrettyDebug(dTimer, "[\t    func-sendRequestVote-rf(%+v)\t\t] :我被杀了",
			rf.me)
		return
	}
	...
	if !ok {
		PrettyDebug(dTimer, "[\t    func-sendRequestVote-rf(%+v)\t\t] :请求服务器超时了:%v,阻塞时长>10ms",
			rf.me, server)
		return
	}
	// 函数执行完成
